...
const statusColor = {
    ...
    [WorkflowStatus.PUBLISHED]: 'bg-primary text-yellow-600'
};
        ...
        <Card className="border border-separate shadow-sm rounded-lg overflow-hidden hover:shadow-md dark:shadow-primary/30">
            <CardContent className="p-4 flex items-center justify-between h-[100px]">
                <div className="flex items-center justify-end space-x-3">
                    ...
                    >
                        {isDraft ? <FileTextIcon className="w-5 h-5" /> : <PlayIcon className="h-5 w-5 text-white" />}
                    </div>
                    <div className="">
...
function WorkflowActions({ workflowName, workflowId }: { workflowName: string; workflowId: string }) {
    ...
    return (
        <>
            <DeleteWorkflowDialog isOpen={showDeleteDialog} setIsOpen={setShowDeleteDialog} workflowName={workflowName} workflowId={workflowId} />
            <DropdownMenu>
